<?php class acf_field_user_network extends acf_field {
	...
	function __construct() {
		// vars
		$this->name     = 'user_network';
		$this->label    = __( "User Network", 'acf-user-network' );
		...
		// extra
		add_action( 'wp_ajax_acf/fields/user_network/query', array( $this, 'ajax_query' ) );
		add_action( 'wp_ajax_nopriv_acf/fields/user_network/query', array( $this, 'ajax_query' ) );
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		// filters
		$args = apply_filters( "acf/fields/user_network/query", $args, $field, $options['post_id'] );
		$args = apply_filters( "acf/fields/user_network/query/name={$field['_name']}", $args, $field, $options['post_id'] );
		$args = apply_filters( "acf/fields/user_network/query/key={$field['key']}", $args, $field, $options['post_id'] );


		$users = get_users( $args );

		// get users
		$groups = ! empty( $users ) ? [ 'All' => $users ] : [];
	...
	*/
	...
	function get_result( $user, $field, $post_id = 0 ) {
		...
		// filters
		$result = apply_filters( "acf/fields/user_network/result", $result, $user, $field, $post_id );
		$result = apply_filters( "acf/fields/user_network/result/name={$field['_name']}", $result, $user, $field, $post_id );
		$result = apply_filters( "acf/fields/user_network/result/key={$field['key']}", $result, $user, $field, $post_id );
	...
	*/
	...
	function user_search_columns( $columns, $search, $WP_User_Query ) {
		...
		$columns = apply_filters( "acf/fields/user_network/search_columns", $columns, $search, $WP_User_Query, $field );
		$columns = apply_filters( "acf/fields/user_network/search_columns/name={$field['_name']}", $columns, $search, $WP_User_Query, $field );
		$columns = apply_filters( "acf/fields/user_network/search_columns/key={$field['key']}", $columns, $search, $WP_User_Query, $field );
